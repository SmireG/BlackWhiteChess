...
class CodeCheck:
    ...
    def time_out_init(self):
        self.agent = imp.load_source("AI6", self.script_file_path).AI(
            ...
            if result is not None:
                for i in result:
                    if tuple(i) not in self.agent.candidate_list:
                        print("User choice {}\nThe answer should be {}".format(self.agent.candidate_list,
                                                                               result.tolist()))
                        return False
...
if __name__ == "__main__":
    code_checker = CodeCheck("AI6.py", 8)  ## change the file name to your python file name
